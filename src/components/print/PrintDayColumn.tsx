...
export function PrintDayColumn({
  ...
  return (
    <div className="flex flex-col flex-1 border-r border-black last:border-r-0 h-full overflow-hidden bg-white text-black">
      {/* Day Header */}
      <div className="border-b-2 border-black bg-white py-1 px-1.5 text-center">
      ...
      {/* Meal Slots */}
      <div className="flex-1 flex flex-col divide-y divide-black overflow-hidden text-[9px] leading-tight">
        {SLOTS.map((slot) => {
          ...
          return (
            <div key={slot} className="flex-1 p-1 flex flex-col min-h-0 bg-white">
              <span className="text-[8px] font-bold uppercase tracking-widest text-black block mb-0.5">
                {slot}
              </span>
              <div className="space-y-0.5 overflow-hidden">
                ...
                {!hasMeals && includeBlankSlots && (
                  <div className="border-b border-dotted border-black mt-2 h-0 w-full" />
                )}
              </div>
            </div>
          );
        ...
        {includeEvents && (dayEvents.length > 0 || includeBlankSlots) && (
          <div className="p-1 min-h-[30px] bg-white border-t border-black">
            <span className="text-[8px] font-bold uppercase tracking-widest text-black block mb-0.5">
              Events
            </span>
            <div className="space-y-0.5">
              {dayEvents.map((evt) => (
                <div key={evt.id} className="text-black text-[8px] truncate flex items-center gap-1">
