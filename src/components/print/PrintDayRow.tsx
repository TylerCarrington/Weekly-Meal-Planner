...
export function PrintDayRow({
  ...
  return (
    <div className="flex border-b border-black last:border-b-0 flex-1 min-h-0 bg-white text-black text-[9px] leading-tight">
      {/* Day label */}
      <div className="w-20 border-r border-black bg-white p-1.5 flex flex-col justify-center text-center shrink-0">
      ...
      {/* Meals Grid */}
      <div className="flex-1 grid grid-cols-4 divide-x divide-black p-1 bg-white">
        {PRIMARY_SLOTS.map((slot) => {
          const slotMeals = dayMeals.filter((m) => m.type === slot);
          return (
            <div key={slot} className="px-1.5 flex flex-col min-h-0">
              <span className="text-[7.5px] font-bold uppercase tracking-widest text-black mb-0.5">
                {slot}
              </span>
      ...
      {includeEvents && (
        <div className="w-28 border-l border-black p-1 bg-white flex flex-col shrink-0">
          <span className="text-[7.5px] font-bold uppercase tracking-widest text-black mb-0.5">
            Events
          </span>
          <div className="space-y-0.5 overflow-hidden">
            {dayEvents.map((evt) => (
              <div key={evt.id} className="text-[8px] truncate font-medium text-black">
